...
def phase_linker_setup(options, state):  # noqa: C901, PLR0912, PLR0915
    ...
    * There are currently 578 statements vs 50 recommended.
    ...
    if not options.oformat and (options.relocatable or options.shared):
    ...
    if not ext:
        wasm_target = f"{target}.exe"
    # if options.oformat in (OFormat.EXE, ):
    #     # if options.emit_tsd:
    ...
    #     wasm_target = get_secondary_target(target, '.exe')
